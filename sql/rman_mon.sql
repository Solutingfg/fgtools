...
prompt
select s.inst_id, o.sid, CLIENT_INFO channel, context, sofar, totalwork,
...
prompt
select inst_id, sid, CLIENT_INFO channel, seq#, event, state, wait_time_micro/1000000 seconds
from gv$session where program like '%rman%' and
wait_time = 0 and
not action is null;

REM use the following for 10G
--select  inst_id, sid, CLIENT_INFO channel, seq#, event, state, seconds_in_wait secs
--from gv$session where program like '%rman%' and
--wait_time = 0 and
--not action is null;
...
prompt
select s.inst_id, a.sid, CLIENT_INFO channel, a.STATUS,
...
prompt
select s.inst_id, a.sid, CLIENT_INFO channel, filename, a.type, a.status, buffer_size bsz, buffer_count bfc, open_time open, io_count
